    ...
    apiClient.request(url)
      .then(data => {
        ...
        if (selectedCommunity) {
          elements = [
            ...data.nodes.map((n: any) => ({ data: { id: n.id, label: n.label, role: n.role, pagerank: n.pagerank, betweenness: n.betweenness } })),
            ...data.edges.map((e: any) => ({ data: { source: e.source, target: e.target, label: e.type } }))
          ];
        } else {
          // Full graph topology mapping
          elements = [
            ...data.nodes.map((n: any) => ({ data: { id: n.id, label: n.primary_name || n.id, role: 'member', pagerank: 1, betweenness: 0 } })),
            ...data.edges.map((e: any) => ({ data: { source: e.source, target: e.target, label: e.type } }))
          ];
        }
