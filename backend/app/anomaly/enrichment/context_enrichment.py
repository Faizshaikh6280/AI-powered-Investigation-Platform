...
class ContextEnrichmentEngine:
    ...
    def _enrich_events_and_timeline(
        ...
        for ev in deduped_events:
            dt = get_dt(ev)
            if t0 is None and dt != datetime.min:
                t0 = dt

            delta_str = "+0m"
            if t0 and dt != datetime.min:
            ...
            if domain == "BANKING" or event_type in ("TRANSACTION", "TRANSFER"):
                ...
                loc_disp = location_str or "Sector 22 Market, Chandigarh"
    ...
    def _enrich_spatial_context(
        ...
        if not waypoints and enriched.supporting_events:
            for ev in enriched.supporting_events:
                ...
                if lat is not None and lng is not None:
                    waypoints.append({
                        "name": loc_name or "Sector 22 Market, Chandigarh",
                        "lat": float(lat),
                        "lng": float(lng),
                        "timestamp": ev.get("timestamp"),
                        "event_id": eid,
        ...
        if waypoints or dist_km is not None:
            enriched.spatial_context = {
                "available": True,
                "waypoints": waypoints,
                "primary_location": waypoints[0].get("name") if waypoints else "Sector 22 Market, Chandigarh",
                "distance_km": dist_km,
                "implied_speed_kmh": speed_kmh,
                "elapsed_seconds": elapsed_sec,
                "movement_description": (
                    f"{dist_km} km traversed in {elapsed_sec}s implying {speed_kmh} km/h ground speed."
                    if dist_km and speed_kmh else f"{len(waypoints)} verified geographic coordinate records at {waypoints[0].get('name', 'Sector 22 Market, Chandigarh')}."
                )
            }
        else:
            enriched.spatial_context = {
