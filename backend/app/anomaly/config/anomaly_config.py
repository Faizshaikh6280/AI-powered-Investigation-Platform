...
class SpatioTemporalThresholds(BaseModel):
    ...
    st_dbscan_eps_time_sec: int = 3600           # Temporal neighborhood window (1 hour)
    ...
    signal_silence_threshold_hours: float = 6.0  # Dark period duration before incident
...
class FinancialThresholds(BaseModel):
    ...
    structuring_min_transactions: int = 3         # Minimum txns in window
    structuring_window_hours: int = 48            # Window to aggregate candidate structuring
    rapid_fanout_window_seconds: int = 1800       # 30-minute inflow to split outflow window
    rapid_fanout_min_counterparties: int = 3      # Minimum outgoing recipients
    rapid_fanout_depletion_ratio: float = 0.80    # Outflow / Inflow ratio threshold
