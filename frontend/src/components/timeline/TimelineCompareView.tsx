...
export const TimelineCompareView: React.FC<CompareViewProps> = ({
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-background overflow-hidden">
      ...
      <div className="flex items-center justify-between px-6 py-3 border-b border-border bg-card/60 backdrop-blur-md gap-4">
        <div className="flex items-center gap-2">
          <ArrowLeftRight className="w-4 h-4 text-primary" />
          <h3 className="text-xs font-bold uppercase tracking-wider text-foreground">Entity Temporal Comparison</h3>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <select
            ...
            defaultValue=""
            className="bg-secondary text-xs text-muted-foreground hover:text-foreground border border-border rounded-md px-2.5 py-1 cursor-pointer outline-none"
          >
      ...
      <div className="flex-1 overflow-y-auto relative p-6 space-y-6">
        {loading ? (
          <div className="py-12 text-center text-xs text-muted-foreground animate-pulse">
            Synchronizing comparison streams across temporal axis...
          </div>
          ...
          <div className="space-y-6 relative">
            ...
            {compareEntities.map((entName, laneIdx) => {
              ...
              return (
                <div key={entName} className="bg-card border border-border rounded-xl p-4 shadow-sm space-y-2">
                  <div className="flex items-center justify-between text-xs">
                    <div className="flex items-center gap-2">
                      <div className="w-2.5 h-2.5 rounded-full bg-primary" />
                      <span className="font-bold text-foreground text-sm">{entName}</span>
                    </div>
                    <span className="font-mono text-muted-foreground">{streamEvents.length} events</span>
                  </div>
